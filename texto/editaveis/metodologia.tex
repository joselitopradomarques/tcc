...
O \textit{mixer} proposto possui sub-blocos de funcionamento e se interligam conforme se encontra na Figura \ref{fig52}.
...
Por fim, os sinais da filtragem e do efeito são somados e convertidos para analógico, de forma que possam ser reproduzidos ao final.

\section{Fluxogramas de Blocos}

Nesta seção, cada bloco presente no Diagrama Geral do Sistema, Fig. \ref{fig52}, será detalhado sobre o seu funcionamento conceitual proposto. 

\subsection{Bloco de Conversão AD}
Esse bloco é responsável pela aquisição e conversão do sinal obtido pelo reprodutor de música, seja uma CDJ, um toca-discos; e convertê-lo para um sinal digital, a fim de que possa ser processado. 
...
\begin{figure}[h]
    ...
    \caption{diagrama para conversão analógico digital}
    \label{fig53}
\end{figure}
...
No canal 2, um novo valor para a frequênca de corte 2 será obtido através da expressão de Equação \ref{eq:05}. Assim, o filtro passa-altas deste canal será atualizado conforme a nova frequência de corte, como se encontra na Figura \ref{fig54}. 

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figuras/fig54.png}
    \caption{processamento dos canais}
    \label{fig54}
\end{figure}

No diagrama da Figura \ref{fig54}, \textit{fc} corresponde à frequência de corte central, ou seja, àquela lida e convertida do botão central; \textit{fc1} é a frequência de corte para o filtro passa-altas 1 (HPF1) e \textit{fc2} é a frequência de corte do canal 2 para o filtro passa-altas 2 (HPF2).

Ao final, os dois canais são combinados e se tem o canal \textit{master}.

\subsection{Bloco de Leitura de Controle de Efeitos}
leitura de parâmetro fx -> atualização de paramnfx
leitura de toogle de fx -> atualização de selfx

\subsection{Bloco de Efeitos - Delay}
conversão de paramnfx para ms -> delayms
leitura de selfx -> 

caso selfx seja delay -> delaysignal com delayms é criado
caso selfx não seja delay -> fazer nada


\subsection{Bloco de Efeitos - Reverb}
conversão de paramnfx para db -> reverdb
leitura de selfx -> 

caso selfx seja reverb -> reverbsignal com reverbdb é criado
caso selfx não seja reverb -> fazer nada
