...
Este projeto propõe a criação de um \textit{mixer} com um controle central que manipula dois arquivos de música. Em vez dos tradicionais controles de três bandas de frequência, o \textit{mixer} utilizará filtros passa-altas. O controle central ajustará simultaneamente as frequências de corte de ambos os canais, permitindo uma transição suave entre as músicas. Além disso, o sistema incluirá dois efeitos, \textit{delay} e \textit{reverb}, que serão controlados através de um botão giratório, que determinará a intensidade do efeito aplicado.
...
\begin{itemize}
    \item %O sistema deve permitir ao \textit{DJ} 
    Realizar a leitura de arquivos de áudio no formato \textit{.wav};
    \item %O sistema deve p
...
% Já nesta subseção, há a descrição de como o sistema deve se comportar 
Em relação ao desempenho, segurança, usabilidade e outros atributos qualitativos, temos os seguintes requisitos não-funcionais:

\begin{itemize}
    \item % O sistema deve responder aos comandos do \textit{DJ} com latência mínima, garantindo uma
    ...
    \item %O sistema deve oferecer uma 
    Qualidade de som de alta fidelidade e com mínimas distorções; %, garantindo que o áudio reproduzido seja claro e com mínimas distorções;
    \item Interface com botões físicos ou controles táteis para ajustes; % de frequência e efeitos de áudio;
    \item Organização lógica e intuitiva da interface, %A interface do mixer deve ser organizada de forma lógica e intuitiva, 
    com controles agrupados por função; % para facilitar a navegação;
    \item %A interface deve possuir 
    Indicações claras das funções dos botões de interação com o usuário;
    \item %O sistema deve incluir 
    Interface de saída de áudio padrão \textit{RCA};
    \item %O sistema deve suportar uma 
    ...
    \item %O sistema deve ser projetado para minimizar o
...
Para aplicar a filtragem, utilizou-se a função \texttt{hip\textasciitilde}, que implementa um filtro passa-altas. No entanto, o argumento da função deve variar entre o canal 1 e o canal 2. Como mostrado na Figura \ref{fig44}, o canal 1 ("FC do HPF1") recebeu diretamente o parâmetro \textit{fc}, proveniente do \textit{slider} azul, e o filtro passa-altas do canal 2 ("FC do HPF2") utilizou um valor ajustado %por uma expressão anterior, 
conforme a Equação \ref{eq:05}, que descreve um círculo com raio igual à frequência de amostragem, com o botão centralizado no ponto (22050, 22050). Esse ajuste assegura que uma pequena variação em \textit{fc$_{1}$} resulte em uma grande variação em \textit{fc$_{2}$}, e vice-versa. Além disso, em frequências centrais, a variação entre os canais torna-se mais semelhante. % A Equação \ref{eq:05} descreve um círculo com raio igual à frequência de amostragem, com o botão centralizado no ponto (22050, 22050).
...
No sistema, a amplitude do efeito é ajustado automaticamente com base na posição do botão central, ou seja, nas frequências de corte. A Figura \ref{fig49} ilustra a variação do volume dos efeitos em função da frequência central. % \newpage
...
% \newpage
\section{Sistema definitivo} %Proposta de Implementação}

Em linhas gerais, %a proposta de implementação desse projeto envolve a utilização de
...
\begin{figure}[htpb]
    ...
    \end{minipage}
    \hspace{2cm} % Espaço horizontal reduzido entre as figuras
    \begin{minipage}[t]{0.3\textwidth}
        \centering
...
% %     \label{fig62}
% % \end{figure}
% A chave foi utilizada para selecionar entre dois efeitos distintos, gerando dois níveis de tensão: um nulo e outro de alimentação. Cada nível corresponderá a um efeito diferente. Um exemplo de botão para essa finalidade é mostrado na Figura \ref{fig61}. %\ref{fig62}.
% Para ajustar a quantidade de efeitos desejada, foi utilizado um botão semelhante ao \textit{slider}, com a mesma abordagem de controle e ajuste.

% \paragraph*{Conversão AD}

A conversão analógica-digital para os \textit{knobs} foi feita %empregada na aquisição dos sinais de controle, ou seja, para dois potenciômetros, 
utilizando um conversor PCF8591 \cite{saravaticomponentes}, % (Fig. \ref{fig64}), 
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.3\textwidth]{figuras/fig64.jpg}
...
%     \caption{ Placa \textit{Raspberry Pi} para processamento dos sinais \cite{adrenalineDisplaysLanar}}
%     \label{fig65}
% \end{figure}
...
%necessário para que a \textit{Raspberry Pi} consiga identificar o dispositivo e receber as leituras realizadas. Naturalmente, esse dispositivo possui endereço \textit{0x48}. Porém, quando os pinos A0, A1 e A2 estavam abertos, havia uma flutuação do endereço. Para estabilizar o endereço, foi necessário o aterramento dessas entradas de endereço. 
...
Outro componente utilizado foi a chave de duas posições, no diagrama \ref{fig80}, denominado de \textit{SW1}. %Essa chave é capaz de mandar ao GPIO da \textit{Raspberry} o nível lógico alto (3V3) ou baixo (GND). Com esses níveis, é possível descobrir se o usuário deseja que o efeito 1 ou o 2 seja executado naquele momento.

% Para a reprodução do sinal de saída, utilizou-se a entrada de 3,5 m nativa da \textit{Raspberry}.
 
\subsection{\textit{Software}}

O processamento de sinais de áudio foi implementado utilizando várias bibliotecas, disponíveis na pasta \textit{scripts}, na \textit{branch main} no \textit{link}: \href{https://github.com/joselitopradomarques/tcc}{Repositório do projeto}, cada uma com funções específicas para cada etapa do processamento: %Inicialmente, realizou-se a leitura de arquivos de áudio no formato \textit{.wav}, segmentados em \textit{buffers} para permitir um processamento contínuo e eficiente.  Em seguida, a filtragem e os efeitos foram aplicados, e os sinais foram reproduzidos. Parâmetros de controle, como frequência de corte e intensidade dos efeitos, foram ajustados em tempo real, de acordo com a aquisição dos sinais de controle:
...
\begin{description}
    ...
    \item [Aquisição] Os sinais de áudio foram adquiridos através da leitura de arquivos \textit{.wav} locais.%, através de  funções principais: \verb|ler_wav_estereo| para ler um arquivo e \verb|ler_dois_wav_estereo| para ler dois arquivos.
...
% Os coeficientes do filtro FIR são pré-calculados para todas as 256 frequências de corte possíveis, evitando o recálculo a cada ciclo.
...
% Para a implementação do processamento de sinais de áudio, o código foi estruturado em várias bibliotecas, cada uma contendo funções específicas para cada etapa do processamento. 
% O fluxo começa com a declaração das variáveis necessárias para iniciar a aquisição dos sinais. Em seguida, os arquivos de áudio no formato \textit{.wav} são lidos e processados utilizando uma lógica de \textit{buffers}, que segmenta os dados para permitir um processamento contínuo e eficiente. 
